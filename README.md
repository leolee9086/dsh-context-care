...
除循环之外，流式管线还带两个**焦虑检测**模式：`time-anxiety` 与 `context-anxiety`。
两者同源 —— 都是先盘算一个资源余量，再从「可能不够」推出「得省着用」：
前者盘算时间和调用次数，后者盘算上下文还剩多少、或者想抢在压缩之前做点什么。

两者都只认思考块（正文里讨论时间成本、上下文占用、压缩策略都是正常工作），
命中后**只提醒、不掐断** —— 念头跑偏不毁坏这一轮，打断反而更糟。

提醒文案走**认知行为疗法**（CBT）的路子，并署名 **Seraph · 系统的心理医生模块**。
这不是装饰：本插件就是 s-forge 的 seraph 在 DSH 的落地载体，判断的权威性来自来源。

- **不反驳结论，拆掉前提。** 旧文案讲后果（「省时间反而更费时间」），但焦虑的起点
  不是不知道后果，是**错误估计了余量**；对一个以为自己快没资源的人讲成本收益，
  等于没答他的问题。时间不是可观测的量，所以「够不够」本身没有答案，
  由它推出来的结论也就不成立。结构对应 CBT 四步：条件式识别 → 证据检验 →
  替代认知 → 行为指令。
- **条件式开头对误报是安全的**：模型没在盘算时假设不成立，剩下的话只是重申工作标准。
- **上下文那条能给出真实数值**（最近一次状态报告的 `fatigue` / `wakefulness`），
  因为上下文占用是有客观测量的 —— 这是 CBT 的证据检验，比时间焦虑只能讲道理更强。
  数值经 `previousState` 同步取得：流式路径挂在同步广播上，不能在那里 await 采样。
- **词表按施动者筛选**：只收把资源当威胁的表述，不收把资源当工具的表述。
  「该压缩了」是健康行为（主动用工具），「趁还没压缩」才是歪曲（把工具当成要躲的事件）。
